...
class Database:
    ...
    def creation(self, directory):
        ...
        '''
        for _, dirs, _ in os.walk(directory, topdown=True):
            for dir in dirs:
                name = dir.replace("_", " ")
                print(name)
                for f in list(os.walk(directory + "/" + dir))[0][2]:
                    print(f)
                    user_input_tuple = userinput(camera=False, image_directory=directory + "/" + dir + "/" + f)
